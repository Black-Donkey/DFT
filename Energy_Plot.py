    ...
    with open(str_path_vaspout, 'r') as file:
        for line in file.readlines():
            ...
            if str_keyword in line.strip():
                ...
                print(msg)
                lst_f = float(linecache.getline(str_path_vaspout, int_line_idx).split()[2].replace("=", ""))
                lst_e0 = float(linecache.getline(str_path_vaspout, int_line_idx).split()[4].replace("=", ""))
                lst_de = float(linecache.getline(str_path_vaspout, int_line_idx).split()[7].replace("=", ""))
